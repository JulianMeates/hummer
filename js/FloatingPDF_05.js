...
// In your createFloatingContainer() function, replace the dragging section with this:
...
createFloatingContainer() {
  ...
  if (!isMobile && !isSmallViewport) {
    // Desktop: Enable dragging with improved drag handle
    this.setupDesktopDragFunctionality(container);
...
// NEW METHOD: Simplified desktop drag functionality
setupDesktopDragFunctionality(container) {
  ...
  // Make the entire container draggable, but prevent dragging from interactive elements
  container.addEventListener('mousedown', (e) => {
    // Don't drag if clicking on interactive elements
    const preventDragElements = ['BUTTON', 'CANVAS', 'IFRAME', 'INPUT', 'SELECT', 'TEXTAREA'];
    if (preventDragElements.includes(e.target.tagName)) {
      return;
    }

    // Don't drag if clicking on PDF content area (where PDF is displayed)
    ...
    if (contentArea && canvasContainer) {
      ...
      if (isInCanvas) {
        return; // Don't drag from PDF viewing area
      }
    }
    ...
    containerStartX = rect.left + rect.width / 2;
    containerStartY = rect.top + rect.height / 2;
  ...
  // Reset cursor when hovering over interactive elements
...
createDesktopControls(totalPages) {
  ...
  if (totalPages > 10) {
    ...
    fastPrevBtn.onmouseover = () => fastPrevBtn.style.background = 'rgba(255,255,255,0.3)';
    fastPrevBtn.onmouseout = () => fastPrevBtn.style.background = 'rgba(255,255,255,0.2)';
  ...
  if (totalPages > 10) {
    ...
    fastNextBtn.onmouseover = () => fastNextBtn.style.background = 'rgba(255,255,255,0.3)';
    fastNextBtn.onmouseout = () => fastNextBtn.style.background = 'rgba(255,255,255,0.2)';
    navGroup.appendChild(fastNextBtn);
  }
  
  // CENTER: Page info and zoom controls
  ...
  const pageDisplay = document.createElement('span');
  pageDisplay.id = 'desktop-page-info';
  pageDisplay.textContent = `Page ${this.currentPage} of ${totalPages}`;
  pageDisplay.style.cssText = `
    font-weight: bold;
    font-size: 14px;
    min-width: 120px;
  ...
  zoomOutBtn.style.cssText = baseButtonStyle + 'width: 32px; padding: 6px;';
  zoomOutBtn.onclick = () => this.adjustDesktopZoom(0.8);
  zoomOutBtn.onmouseover = () => zoomOutBtn.style.background = 'rgba(255,255,255,0.3)';
  zoomOutBtn.onmouseout = () => zoomOutBtn.style.background = 'rgba(255,255,255,0.2)';
  
  const zoomResetBtn = document.createElement('button');
  zoomResetBtn.textContent = '100%';
  zoomResetBtn.title = 'Reset zoom';
  zoomResetBtn.style.cssText = baseButtonStyle + 'min-width: 50px;';
  zoomResetBtn.onclick = () => this.resetDesktopZoom();
  zoomResetBtn.onmouseover = () => zoomResetBtn.style.background = 'rgba(255,255,255,0.3)';
  zoomResetBtn.onmouseout = () => zoomResetBtn.style.background = 'rgba(255,255,255,0.2)';
  
  const zoomInBtn = document.createElement('button');
  zoomInBtn.textContent = '+';
  zoomInBtn.title = 'Zoom in';
  zoomInBtn.style.cssText = baseButtonStyle + 'width: 32px; padding: 6px;';
  zoomInBtn.onclick = () => this.adjustDesktopZoom(1.25);
  zoomInBtn.onmouseover = () => zoomInBtn.style.background = 'rgba(255,255,255,0.3)';
  zoomInBtn.onmouseout = () => zoomInBtn.style.background = 'rgba(255,255,255,0.2)';
  
  pageInfo.appendChild(zoomOutBtn);
  pageInfo.appendChild(pageDisplay);
  pageInfo.appendChild(zoomResetBtn);
  pageInfo.appendChild(zoomInBtn);
  ...
  minimizeBtn.style.cssText = `
    ...
    transition: background 0.2s;
  `;
  minimizeBtn.onclick = () => this.minimizeFloatingPDF();
  minimizeBtn.onmouseover = () => minimizeBtn.style.background = 'rgba(255, 193, 7, 1)';
  minimizeBtn.onmouseout = () => minimizeBtn.style.background = 'rgba(255, 193, 7, 0.9)';
  ...
  closeBtn.style.cssText = `
    ...
    transition: background 0.2s;
  `;
  closeBtn.onclick = () => this.closeFloatingPDF();
  closeBtn.onmouseover = () => closeBtn.style.background = 'rgba(220, 53, 69, 1)';
  closeBtn.onmouseout = () => closeBtn.style.background = 'rgba(220, 53, 69, 0.9)';
  ...
  controlsDiv.appendChild(centerGroup);
