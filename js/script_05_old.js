...
function loadLogon() {
  ...
  $.postJSON_JGS(url, json, function(loadLogonData){
    var lJadeData = loadLogonData; // atob (loadLogonData);
...
function submitFileSplit (n){
  ...
  function submitFile_SendChunk(pChunk){
    ...
    .fail(function(jqXHR, status, error){
      ...
      document.getElementById("Footer").innerHTML = "<div class='ftrError'>Sorry - we are unable to process your request - please contact Torro Softwarex" + error + "</div>";
      glbSubmitted = false;
      window.document.body.style.cursor = "auto"; 
      return lResponse;
    });
...
// object
  ...
  jsonRequestLogin (json)

    .then (response =>  
    {
     
        var xJadeData = response; // response.substring (1,response.length - 1);
       ...
       //  console.log('Section string: ' + lSectionHtml);
         if (lSectionHtml != "" ){
     ...
     //           document.getElementById("SA_Side").style.display = "none";
      ...
      document.getElementById("Footer").innerHTML = "<div class='ftrError'>Sorry - we are unable to process your request - please contact Torro Software " + error + "</div>";
       glbSubmitted = false;
...
function submitValue_Web (n) {
  ...
  function jsonRequestLogin(formData, options = {}) {
  // Default options
  const config = {
...
/**
 * jQuery-like $.ajax function for JSON requests
...
function jsonAjax(options) {
  return new Promise((resolve, reject) => {
    ...
    const log = debug ? console.log.bind(console, '[jsonAjax]') : () => {};
    
    // Normalize data
    const normalizedData = {};
    Object.keys(data).forEach(key => {
      if (typeof data[key] === 'string') {
        normalizedData[key] = data[key].replace(/[\r\n]/g, '');
      } else {
        normalizedData[key] = data[key];
      }
    });
    
    // Serialize data based on contentType
    let bodyData;
    const contentType = options.contentType || 'application/json';
    
    if (contentType.includes('application/json')) {
      bodyData = JSON.stringify(normalizedData);
    } else {
      // Fall back to form-urlencoded
      const params = new URLSearchParams();
      Object.keys(normalizedData).forEach(key => {
        params.append(key, normalizedData[key]);
      });
    ...
    fetch(url, {
      ...
      headers: {
        ...
        'Accept': options.dataType === 'json' ? 'application/json' : '*/*',
        'X-Requested-With': 'XMLHttpRequest'
      },
