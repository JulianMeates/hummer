...
function registerPhotoWithServerxxxx(phoneImageId) {

   const photoData = getPhotoByPhoneId(phoneImageId);
    if (!photoData) {
        throw new Error('Photo not found in local storage');
    }
    console.log('Full photoData structure:', JSON.stringify(photoData, null, 2));

    var f = document.getElementById(glbBaseFormName);
    var lJson = JSON.stringify({
                filename: Conversions.base32.encode(`photo-${Date.now()}-${phoneImageId}.jpg`),
                vin: Conversions.base32.encode(photoData.metadata.size.toString()),
                phoneImageId: phoneImageId,
...
async function registerPhotoWithServer(phoneImageId) {
    try {
        ...
        window.document.body.style.cursor = "auto";
        document.getElementById("Footer").innerHTML = 
            "<div class='ftrError'>Sorry - we are unable to process your request - please contact Torro Software</div>";
