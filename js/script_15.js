...
async function registerPhotoWithServer(phoneImageId) {
    try {
        ...
        const size = metadata.size || photoData.size || 0;
...
*/
        ...
        const payload = {
            filename: Conversions.base32.encode(`photo-${Date.now()}-${phoneImageId}.jpg`),
            vin: Conversions.base32.encode(size.toString()),
            phoneImageId: phoneImageId,
            size: size,
            mimeType: metadata.mimeType || 'image/jpeg',
            quality: metadata.quality || 100,
...
*/
        ...
        // Finalize upload
      //  await finalizeUpload(fileHandle);
...
async function sendChunkToServer(fileHandle, chunkIndex, chunkDataURL) {
    const payload = {
        ...
        sid : Conversions.base32.encode(chunkIndex.toString())
      };
...
// Function to finalize the upload (tell server all chunks are sent)
async function finalizeUpload(fileHandle) {
    const payload = {
        fileHandle: fileHandle,
        formName: "FileUploadComplete" // Or whatever your server expects
    };
    ...
    if (!result.success) {
        throw new Error(`Upload finalization failed: ${result.error || 'Unknown error'}`);
    }
...
async function uploadChunk(fileHandle, chunkIndex, chunkData) {
    const payload = {
        systemName: glbSystemName,
...
async function checkForStoredImages() {
    try {
        ...
        if (photos.length > 0) {
            // Show the upload images option
            showUploadImagesOption(photos.length);
            // Update the footer stats
            updateFooterStats(photos.length);
        } else {
            // Hide the upload images option if it exists
...
async function updateFooterStats(pendingCount) {
    const statLabel = document.querySelector('.stat-label');
    if (statLabel) {
        try {
            // Get more detailed stats from your database
            const photos = await getAllStoredPhotos();
            
            // Pending: captured (not registered) + registered but not uploaded
            const pending = photos.filter(photo => 
                photo.metadata && (
                    photo.metadata.status === 'captured' || 
                    (photo.metadata.status === 'registered' && !photo.metadata.uploaded)
                )
            ).length;
            
            // Uploaded: photos that have been successfully uploaded
            const uploaded = photos.filter(photo => 
                photo.metadata && photo.metadata.uploaded
            ).length;
            
            // Failed: photos with failed status or other error states
            const failed = photos.filter(photo => 
                photo.metadata && photo.metadata.status === 'failed'
            ).length;
            
            statLabel.textContent = `Pending ${pending}  Uploaded ${uploaded}  Failed ${failed}`;
            
        } catch (error) {
            // Fallback to simple count
            statLabel.textContent = `Pending ${pendingCount}  Uploaded 0  Failed 0`;
        }
    }
}
