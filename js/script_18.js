...
function submitFile_DestinationHandle() {
    ...
    .then(response => {
        try {
            ...
            hideProcessing();
            window.document.body.style.cursor = "auto"; 
            document.getElementById("Footer").innerHTML = "<div class='ftrError'>Error parsing server response - please contact Torro Software</div>";
        }
    })
    .catch(error => {
...
function submitFile_NextChunk() {
    ...
    reader.onerror = function(error) {
        ...
        hideProcessing();
        window.document.body.style.cursor = "auto"; 
        document.getElementById("Footer").innerHTML = "<div class='ftrError'>Error reading file chunk - please contact Torro Software</div>";
    };
}
...
function submitFile_SendChunk(pChunk, pChunkNumber, pNumberOfChunks) {
        ...
        try {
            ...
            if (Object.keys(lJadeData).length > 0) {
                ...
                if (lStatus === "Complete" || pChunkNumber >= pNumberOfChunks) {
                    ...
                    hideProcessing();
                    window.document.body.style.cursor = "auto"; 
                    document.getElementById("Footer").innerHTML = "<div class='ftrError'>Server error processing chunk - please contact Torro Software</div>";
                ...
                hideProcessing();
                window.document.body.style.cursor = "auto"; 
                document.getElementById("Footer").innerHTML = "<div class='ftrError'>Empty server response - please contact Torro Software</div>";
            }
        } catch (error) {
    ...
    .catch(error => {
        ...
        document.getElementById("Footer").innerHTML = "<div class='ftrError'>Network error sending chunk - please contact Torro Software: " + error + "</div>";
        glbSubmitted = false;
        hideProcessing();
        window.document.body.style.cursor = "auto"; 
    });
...
async function storeImageLocally(imageBlob, metadata) {
    ...
    try {
        ...
        const photoData = {
              ...
              metadata: {
                  ...metadata,
                  size: imageMetadata.size, // Use the ACTUAL processed size, not original
                  originalSize: metadata.size, // Keep original size for reference
                  compressed: imageMetadata.size !== metadata.size,
...
async function uploadImageToServer(phoneImageId) {
    try {
        ...
        // Upload chunks with proper error handling and status checking
        let lByteIndex = 0;
        let lChunkStatus = null; // DECLARE THE VARIABLE HERE
        ...
        for (let lChunkNumber = 1; lChunkNumber <= numberOfChunks; lChunkNumber++) {
            ...
            try {
                ...
                console.log(`Chunk ${lChunkNumber} data length: ${chunkDataURL.length}`);
                ...
                console.log(`Chunk ${lChunkNumber} response:`, lJadeData);
                
                // Check if response has data
                if (Object.keys(lJadeData).length > 0) {
                    lChunkStatus = lJadeData.status;
                    console.log(`Chunk ${lChunkNumber} status: ${lChunkStatus}`);
                    ...
                    console.warn(`Empty response for chunk ${lChunkNumber}`);
                }
        ...
        // Check final status after all chunks
        console.log(`All chunks sent. Final status: ${lChunkStatus}`);
        
        if (lChunkStatus === "Complete") {
            console.log('Upload completed successfully, deleting from local storage');
            await deletePhotoFromLocalStorage(phoneImageId);
        } else {
            console.warn(`Upload finished but status is: ${lChunkStatus}`);
        }
