...
async function uploadImageToServer(phoneImageId) {
    try {
        ...
        if (!photoData) {
            throw new Error('Photo not found in OPFS');
        }
        ...
        if (!photoData.metadata.fileHandle) {
            ...
            const updatedPhotoData = await photoStorage.getPhoto(phoneImageId);
            if (!updatedPhotoData.metadata.fileHandle) {
        ...
        if (lChunkStatus === "Complete") {
            console.log('Upload completed successfully, deleting from OPFS');
...
// OPFS-Only Photo Storage - Clean Implementation
class OPFSPhotoStorage {
    constructor() {
        this.opfsRoot = null;
        this.isReady = false;
    }
    ...
    async init() {
        try {
            if ('storage' in navigator && 'getDirectory' in navigator.storage) {
                ...
                this.isReady = true;
                console.log('✅ OPFS Photo Storage initialized');
                return true;
            } else {
    ...
    async savePhoto(phoneImageId, imageBlob, metadata) {
        ...
        try {
            // Save the image file
            const imageHandle = await this.opfsRoot.getFileHandle(`${phoneImageId}.jpg`, { create: true });
            ...
            const metaHandle = await this.opfsRoot.getFileHandle(`${phoneImageId}.json`, { create: true });
            const metaWritable = await metaHandle.createWritable();
            await metaWritable.write(JSON.stringify(enhancedMetadata, null, 2));
    ...
    async getPhoto(phoneImageId) {
        ...
        try {
            // Get image file
            const imageHandle = await this.opfsRoot.getFileHandle(`${phoneImageId}.jpg`);
            const imageFile = await imageHandle.getFile();

            // Get metadata
            const metaHandle = await this.opfsRoot.getFileHandle(`${phoneImageId}.json`);
            ...
            console.warn(`Photo not found: ${phoneImageId}`);
            return null;
        }
    }
        ...
        try {
            for await (const [name, handle] of this.opfsRoot.entries()) {
                if (name.endsWith('.jpg')) {
                    const phoneImageId = name.replace('.jpg', '');
                    const photo = await this.getPhoto(phoneImageId);
    ...
    async updatePhoto(phoneImageId, updates) {
        ...
        try {
            const existing = await this.getPhoto(phoneImageId);
            if (!existing) {
                console.warn(`Cannot update - photo not found: ${phoneImageId}`);
                return false;
            }
    ...
    async deletePhoto(phoneImageId) {
        ...
        try {
            await this.opfsRoot.removeEntry(`${phoneImageId}.jpg`);
            await this.opfsRoot.removeEntry(`${phoneImageId}.json`);
            console.log(`🗑️ Deleted: ${phoneImageId}`);
            checkForStoredImages ();
            return true;
        } catch (error) {
            console.error(`❌ Delete failed: ${phoneImageId}`, error);
        ...
        try {
            ...
            for await (const [name, handle] of this.opfsRoot.entries()) {
                if (name.endsWith('.jpg') || name.endsWith('.json')) {
                    ...
                    if (name.endsWith('.jpg')) deletedCount++;
                }
            }
            ...
            console.log(`🧹 Cleared ${deletedCount} photos`);
        } catch (error) {
...
const photoStorage = new OPFSPhotoStorage();

// Replace your existing IndexedDB functions with these OPFS versions
