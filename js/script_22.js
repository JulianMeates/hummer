...
// Enhanced PDF.js implementation for mobile (replace your displayWithPDFJS method)
async displayWithPDFJS(blobUrl, container) {
  try {
    ...
    // Create canvas container with touch support
    ...
    // Create scrollable wrapper for zoom and pan
...
// Setup touch interactions for swipe and zoom
setupTouchInteractions(container, canvas, scrollWrapper) {
  ...
  container.addEventListener('touchend', (e) => {
    if (isSwipeGesture && e.changedTouches.length === 1) {
      ...
      // Check for swipe gesture (horizontal swipe, not too slow, not too short)
...
setupOrientationHandling(container) {
  const handleOrientationChange = () => {
    ...
    setTimeout(async () => {
      // Reset any transforms that might cause misalignment
...
repositionAfterOrientation(container) {
  ...
  if (floatingContainer) {
    ...
    // Ensure it fits in new viewport
    this.keepContainerInBounds(floatingContainer);
  }
}

// Enhanced mobile page rendering
async renderMobilePage(pageNum) {
  ...
  try {
    ...
    // Scale to fit width, but ensure good readability
...
// Navigate to specific page (mobile version)
