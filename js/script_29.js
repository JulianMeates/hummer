    ...
    const makeRequest = (data, retries) => {
        return new Promise((resolve, reject) => {
            ...
            if (config.forceXHR || !('fetch' in window)) {
                xhrRequest(jsonData, controller, timeoutId)
                    .then(resolve)
                    .catch(error => {
               ...
               fetch(config.url, {
                      ...
                      headers: {
                          ...
                          'Pragma': 'no-cache',
                          'Connection': 'close', // ← Add this line
...
class SmartSacrificeManager {
    ...
    async makeSacrifice(url) {
        ...
        try {
            await fetch(url, {
                ...
                body: JSON.stringify({ sacrifice: true }),
    ...
    recordFailure() {
        if (this.isIOS) {
            this.consecutiveSuccesses = 0;
            console.log('📱 Failure recorded - resetting success count');
        }
    }
}
