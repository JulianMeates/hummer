...
async function uploadLater() {
    ...
    try {
        ...
        const metadata = {
            ...
            quality: imageQualitySettings.quality,
            mimeType: imageQualitySettings.mimeType,
            ...capturedPhoto.compressionInfo
        };
        
        // First: Register photo with server to get upload details
        uploadLaterBtn.textContent = 'Getting upload details...';
        const serverDetails = await registerPhotoWithServer(metadata);
        
        // Store image locally with server details
        uploadLaterBtn.textContent = 'Saving locally...';
        const photoId = await storeImageLocallyWithServerDetails(
        ...
        if (connectionInfo.isOnline && (connectionInfo.isWifi || !wifiUploadEnabled)) {
            ...
            showSuccessMessage(`Photo registered and queued for upload! (${(capturedPhoto.photoBlob.size / (1024 * 1024)).toFixed(2)}MB)`);
        } else {
        ...
        // Fallback: Store without server details for retry later
        try {
            uploadLaterBtn.textContent = 'Saving for retry...';
            await storeImageLocallyWithoutServer(capturedPhoto.photoBlob, metadata);
            showError('Server unavailable. Photo saved for registration retry.');
        } catch (fallbackError) {
...
// Upload Now - Direct upload using your Jade chunked system
async function uploadNow() {
    ...
    try {
        ...
        const metadata = {
            ...
            camera: currentFacingMode,
            size: capturedPhoto.photoBlob.size,
            fileSize: capturedPhoto.photoBlob.size,
            fileSizeFormatted: formatFileSize(capturedPhoto.photoBlob.size),
        ...
        // Step 1: Get file destination handle from server
        uploadNowBtn.textContent = 'Getting upload details...';
        const serverDetails = await getFileDestinationHandle(capturedPhoto.photoBlob, metadata);
        
        // Step 2: Upload file in chunks directly
        uploadNowBtn.textContent = 'Uploading...';
        const uploadResult = await uploadPhotoDirectly(capturedPhoto.photoBlob, serverDetails, metadata);
        
        // Step 3: Submit form to complete the process
        await submitPhotoForm(uploadResult.fileHandle, metadata);
        ...
        // Check if it's a network error - if so, queue for later
        if (!connectionInfo.isOnline || error.message.includes('network') || error.message.includes('fetch')) {
            showError('Upload failed due to connection issues. Saving for later upload...');
            try {
                await uploadLater();
                return;
            } catch (queueError) {
                showError('Failed to save photo for later upload.');
            }
        } else {
...
async function uploadNow() {
    const capturedPhoto = document.getElementById('capturedPhoto');
    if (!capturedPhoto.photoFile) return;
    
    // Check WiFi preference
    if (wifiUploadEnabled && !connectionInfo.isWifi && connectionInfo.isOnline) {
        if (!confirm('You\'re on mobile data. Upload anyway?')) {
            return;
        }
    }
    ...
    try {
        uploadNowBtn.disabled = true;
        uploadLaterBtn.disabled = true;
        uploadNowBtn.textContent = 'Registering...';
        

        const metadata = {
            timestamp: new Date().toISOString(),
            timing: "FileDestinationHandle",
            camera: currentFacingMode,
            size: capturedPhoto.photoFile.size,
            uploadType: 'immediate',
            connectionType: connectionInfo.type,
            quality: imageQualitySettings.quality,
            mimeType: imageQualitySettings.mimeType,
        ...
        // Create temporary photo data for upload
        const photoData = {
            imageBlob: capturedPhoto.photoBlob,
            metadata,
            serverDetails
        };
        
        // Upload immediately
        const result = await uploadRegisteredPhoto(photoData);
        
        showSuccessMessage(`Photo uploaded successfully! (${(capturedPhoto.photoFile.size / (1024 * 1024)).toFixed(2)}MB)`);
        retakePhoto();
...
// Enhanced database schema - you might need to update the version
async function initDB() {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open('PhotoQueue', 2); // Increment version
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains('pendingUploads')) {
                ...
                store.createIndex('uploadId', 'serverDetails.uploadId', { unique: false });
            } else if (event.oldVersion < 2) {
                // Add new index for uploadId if upgrading
                const transaction = event.target.transaction;
                const store = transaction.objectStore('pendingUploads');
                if (!store.indexNames.contains('uploadId')) {
                    store.createIndex('uploadId', 'serverDetails.uploadId', { unique: false });
                }
            }
        };
...
async function uploadNow() {
    ...
    try {
        ...
        // Get updated photo data for upload
        const photoData = await getPhotoByPhoneId(capturedPhoto.phoneImageId);
        
        // Upload using server unique ID
        const uploadResult = await uploadRegisteredPhotoWithServerID(photoData);
        
        // Update status to uploaded
        await updatePhotoStatus(capturedPhoto.phoneImageId, 'uploaded');
